		...
		for i in $(cat /config/extended/cache/tidal/$tidalArtistId-videos.json | jq -r '.items | sort_by(.duration) | .[].id'); do
			...
			if find "$lidarrArtistPath" -type f -regex ".*- $tidalVideoTitle\.\(flac\|opus\|m4a\|mp3\)" | read; then
				...
				if [ ! -f "$fileDirectory/$fileNameNoExt.mkv" ]; then
					if [ ! -d /lidarr-extended/music-videos ]; then
						mkdir -p /lidarr-extended/music-videos
						chmod 777 /lidarr-extended/music-videos
						chown abc:abc /lidarr-extended/music-videos
					else
						rm -rf /lidarr-extended/music-videos/*
					fi

					tidal-dl -r P1080
					tidal-dl -o /lidarr-extended/music-videos -l "https://tidal.com/browse/video/$i"

					find "/lidarr-extended/music-videos" -type f -iname "*.mp4" -print0 | while IFS= read -r -d '' video; do
						ffmpeg \
						-i "${video}" \
						-vcodec copy \
						-acodec copy \
